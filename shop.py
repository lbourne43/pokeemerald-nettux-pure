        ...
        for numa in nums:
            if y <= x:
                for key in inv_types:
                    if compiled_inv[num][key] is None:
                        compiled_inv[num][key] = list()
                    if data[name][numa] is not None and key in data[name][numa]:
                        compiled_inv[num][key] += data[name][numa][key]
            y += 1
        x += 1
    for num in nums:
